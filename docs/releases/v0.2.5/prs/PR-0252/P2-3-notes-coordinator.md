...
NotesCoordinator <300 行，持有全部 6 个 manager（NoteSaveTracker, WorkspaceTreeManager, NoteDraftManager, NoteTagManager, NoteTabManager, NoteListManager）。

6 个消费者文件全部从 `_controller` 迁移到 `_coordinator`。迁移完成后，原 `notes_controller.dart` 删除。
...
- Coordinator <300 行，持有全部 6 个 manager
- 迁移 6 个消费者文件：
...
| D7 | `rg -n "features/workspace" apps/lazynote_flutter/lib/features/notes/` | 零匹配 |
