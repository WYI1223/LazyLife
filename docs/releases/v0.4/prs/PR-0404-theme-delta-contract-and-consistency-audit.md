# PR-0404: Theme Delta Contract 定稿与 Repo-wide 一致性审计
...
在 PR-0403 完成 per-ADR 串行全链执行后，基于实际执行经验定稿 Theme Delta Contract
...
1. 新增 ADR 或修改已有 ADR 正文（由 PR-0403 负责）
2. 治理激活声明（由 PR-0405 负责）
3. 模板最终定稿与沉淀到 `docs/development/report-templates/`（由 PR-0406 负责）
...
基于 PR-0403 实际执行经验，将 prep 层的 `governance-theme-delta-contract-model.md`
定稿为可执行的合同模型。
...
- PR-0403 各 iteration 的实际 theme delta 记录

### Action 2: Repo-wide 一致性审计

按 DI-20 T6 裁决的四层检查模型，对 PR-0401~03 全部产出执行跨阶段、跨 ADR 审计：
...
- `governance-source-corpus-inventory-template.zh-CN.md`（PR-0401 规划 + 使用）
- `governance-decision-node-ledger-template.zh-CN.md`（PR-0401 规划 + PR-0403 classification 填充）
- `governance-theme-map-template.zh-CN.md`（PR-0401 规划 + PR-0403 TH 填充）

**已由 Action 1 覆盖**：
- `governance-theme-delta-contract-template.zh-CN.md`（Action 1 模型定稿即为该模板的起草产出）

**规划**（确认字段与章节结构，不写正文，起草由 PR-0405 负责）：
...
| 2 | Consistency Audit Report | 四层检查结果 + Semantic Review open items（传递给 PR-0405） |
