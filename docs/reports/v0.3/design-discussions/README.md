...
| DI-1 | [DI-1-editor-shell-service.md](DI-1-editor-shell-service.md) | EditorShellService 接口 + 状态归属 | D1+D2+D3 | OPEN |
| DI-2 | [DI-2-layout-tree-structure.md](DI-2-layout-tree-structure.md) | 递归布局树节点结构 + 约束传播 | D5+D6 | OPEN |
| DI-3 | [DI-3-layout-persistence.md](DI-3-layout-persistence.md) | 布局持久化、迁移、深度限制 | D7+D8+D9 | OPEN |
...
DI-0 (事实澄清，无依赖)
  ↓
DI-1 (EditorShellService，v0.3 核心接口)
  ↓
DI-2 → DI-3 (布局树结构 → 布局持久化)
  ↓
