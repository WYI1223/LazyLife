...
建立 ADR 层的接收结构与历史补录元数据合同，为 PR-GOV-03 的 per-ADR 串行全链执行
提供基础设施。本 PR 不预设 TH 占位或 ADR Slot，这些由 PR-GOV-03 的 per-ADR
classification 自然产生。
...
1. Theme Map 条目创建（由 PR-GOV-03 classification 产生）
2. ADR Slot / Carrier State 占位（由 PR-GOV-03 自然产生）
3. ADR 正文编写
4. DN classification
...
模板是合同的可执行骨架（fillable markdown structure），起草由 PR-GOV-03 负责，定稿由 PR-GOV-06 负责。

变更约束：元数据合同定稿后，若 PR-GOV-03 执行中发现合同不足（字段缺失、章节结构
无法承载实际内容等），须暂停流程并显式声明 deviation，而非静默回改 PR-GOV-02 产出物。
...
- [PR-GOV-01-source-corpus-and-dn-extraction.md](PR-GOV-01-source-corpus-and-dn-extraction.md)（前置依赖）
