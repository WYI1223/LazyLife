# PR-2: ScopedAtomQuery + ScopedQueryRepository
...
引入统一查询引擎 ScopedQueryRepository，支持按 workspace 子树范围查询 atoms。替代 TaskService/CalendarService 的直查 atoms 路径，为 PR-4 FFI 层的 `query_atoms` 提供后端。

前置条件：PR-1（需要 Migration 0012 的 schema）
...
| DI 裁决 | `docs/reports/v0.3/design-discussions/DI-18-execution-plan.md` Q1（PR-2 行）、Q4（Service 测试） | PR 定位、测试要求（契约真值表） |
