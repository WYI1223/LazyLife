# PR-6: Flutter Features 适配 + 旧 FFI 移除（Contract 阶段）
...
前置条件：PR-5（需要 WorkspaceTreeService B+ 已就位）
...
| DI 裁决 | `docs/reports/v0.3/design-discussions/DI-18-execution-plan.md` Q1（PR-6 行）、Q2（A+ R2/R4 contract 规则）、Q4（清理验证 gate） | PR 定位、迁移策略、清理验证要求 |
...
Out of scope:
- Rust Core 层变更（PR-1~3 已完成）
- Guard/FFI 新增（PR-4 已完成）
- WorkspaceTreeService 基础设施（PR-5 已完成）

## Design

TBD — kickoff 阶段细化。

**Contract 阶段**：本 PR 是 expand-contract 的 contract 阶段。PR-4 expand 阶段保留的旧 FFI 薄 wrapper，在本 PR 中全部移除。Flutter 消费方完成迁移后，旧接口不再有调用方。
