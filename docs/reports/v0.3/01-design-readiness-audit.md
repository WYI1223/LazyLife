...
| D7 | 布局持久化策略 | DI-3 | 0301 | P1 |
| D8 | 扁平→树迁移策略 | DI-3 | 0301 | P1 |
| D9 | 最大深度/pane 数限制 | DI-3 | 0301/0302 | P1 |
| D10 | Buffer 同步模型选型 | DI-4 | 0303/0305 | P0 |
| D11 | 同步粒度 | DI-4 | 0303/0305 | P1 |
| D12 | 光标独立性 | DI-5 | 0303 | P2 |
| D13 | 冲突处理策略 | DI-5 | 0303 | P1 |

### 7.3 更新后的 Planning Phase 关键路径

```
                ┌─ DI-2 (布局树) ─── DI-3 (布局持久化) ──► PR-0301/0302 spec
P1 (Phase 0     │
 spec 4个) ─────┤
                │                                          ┌─► PR-0301B spec
                └─ DI-1 (EditorShell) ─────────────────────┤
                         ↓                                 ├─► PR-0304 spec
                   DI-4 (Buffer 同步) ─ DI-5 (光标/冲突) ──┤
                                                           └─► PR-0303 spec

并行：P1 + DI-1 + DI-2 可同时启动
串行：DI-4 必须在 DI-1 之后；DI-3 在 DI-2 之后；DI-5 在 DI-4 之后
```
