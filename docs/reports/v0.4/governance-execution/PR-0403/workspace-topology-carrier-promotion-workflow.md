...
| `migration-protection` | `PR-0408`, `PR-0410` | `landed` | `crates/lazynote_core/src/db/migrations/0012_workspace_single_root.sql`; `crates/lazynote_core/tests/migration_0012_test.rs`; `crates/lazynote_core/src/service/tree_service.rs`; `crates/lazynote_core/src/repo/workspace_meta_repo.rs`; `crates/lazynote_core/tests/workspace_tree.rs` | schema/trigger guards landed in `PR-0408`, and `PR-0410` now lands matching runtime protection for workspace-root delete/move, designated-folder delete, same-workspace reassign, and cross-workspace move rejection |
| `scoped-query` | `PR-0409` | `landed` | `crates/lazynote_core/src/repo/scoped_query_repo.rs`; `crates/lazynote_core/src/service/task_service.rs`; `crates/lazynote_core/tests/scoped_query_repo.rs`; `crates/lazynote_core/tests/time_matrix.rs`; `crates/lazynote_ffi/src/api.rs` | canonical subtree-scoped query engine is landed; `TaskService` now delegates read paths through `ScopedQueryRepository`, with an explicit pre-`PR-0410` default-workspace compatibility bridge for current root-scoped refs |
| `service-routing` | `PR-0410` | `landed` | `crates/lazynote_core/src/repo/tree_repo.rs`; `crates/lazynote_core/src/service/tree_service.rs`; `crates/lazynote_core/src/service/creation_service.rs`; `crates/lazynote_core/src/repo/workspace_meta_repo.rs`; `crates/lazynote_core/tests/workspace_tree.rs`; `crates/lazynote_core/tests/creation_service_routing.rs` | canonical node-based path/ref-location helpers, designated reassign, protected move/delete rules, designated creation routing, and `origin_workspace_id` write-path are now in-repo |
| `guarded-ffi` | `PR-0411` | `landed` | `crates/lazynote_core/src/guard/mod.rs`; `crates/lazynote_core/src/service/guarded_query_service.rs`; `crates/lazynote_core/src/service/guarded_creation_service.rs`; `crates/lazynote_core/src/service/guarded_atom_service.rs`; `crates/lazynote_core/src/service/guarded_task_service.rs`; `crates/lazynote_core/src/service/guarded_tree_service.rs`; `crates/lazynote_core/src/service/guarded_workspace_service.rs`; `crates/lazynote_ffi/src/api.rs`; `crates/lazynote_core/tests/guard_test.rs` | guarded caller contract, guarded facades, new guarded exports, and expand-stage legacy-wrapper delegation are now in-repo |
| `flutter-core` | `PR-0412` | `landed` | `apps/lazynote_flutter/lib/core/workspace/workspace_tree_types.dart`; `apps/lazynote_flutter/lib/core/workspace/workspace_tree_service.dart`; `apps/lazynote_flutter/lib/features/notes/notes_coordinator.dart`; `apps/lazynote_flutter/lib/features/notes/notes_coordinator_impl.dart`; `apps/lazynote_flutter/lib/features/notes/notes_coordinator_defaults.dart`; `apps/lazynote_flutter/lib/features/notes/note_explorer.dart`; `apps/lazynote_flutter/test/core/workspace/workspace_tree_service_test.dart`; `apps/lazynote_flutter/test/notes_controller_workspace_tree_guards_test.dart`; `apps/lazynote_flutter/test/explorer_context_actions_test.dart`; `apps/lazynote_flutter/test/note_explorer_tree_test.dart`; `apps/lazynote_flutter/test/workspace_integration_flow_test.dart`; `docs/releases/v0.4/prs/PR-0412-flutter-core.md` | guarded workspace caller plumbing, designated-node preload, synchronous system-node lookup, WorkspaceTreeService B+ shape, targeted mutation-delta ownership, and the narrow smoke-driven `NoteExplorer` concrete-root operability bridge are now landed in Flutter core, explicitly consuming `OI-035`, `OI-036`, `OI-038`, `OI-039`, `OI-040`, `OI-042`, `OI-045`, and `OI-048` |
| `flutter-features` | `PR-0413` | `partial` | `docs/releases/v0.4/prs/PR-0413-flutter-features.md`; `apps/lazynote_flutter/lib/features/notes/note_explorer.dart`; `apps/lazynote_flutter/test/explorer_context_actions_test.dart`; `apps/lazynote_flutter/test/note_explorer_tree_test.dart` | broader feature adoption, synthetic removal, controller migration, rename UX, and end-to-end workspace consumers remain pending; only the smoke-driven concrete-root move/create bridge and minimal mutation-delta consumer behavior landed early in `PR-0412` to keep post-`0012` Explorer operable |
| `security-surface` | `PR-0411` and later security work | `partial` | `crates/lazynote_core/src/guard/mod.rs`; `crates/lazynote_core/tests/guard_test.rs`; `crates/lazynote_ffi/src/api.rs`; `docs/releases/v0.4/prs/PR-0411-guard-ffi.md` | guard scaffolding, caller plumbing, and denial-path verification are landed, but default runtime still uses `NoopGuard`, so production enforcement remains deferred |
| `execution-order` | `PR-0408` through `PR-0413` | `partial` | `docs/releases/v0.4/prs/PR-0408-schema-migration.md`; `docs/releases/v0.4/prs/PR-0409-scoped-query-repository.md`; `docs/releases/v0.4/prs/PR-0410-tree-service-creation-service.md`; `docs/releases/v0.4/prs/PR-0411-guard-ffi.md`; `docs/releases/v0.4/prs/PR-0412-flutter-core.md`; `crates/lazynote_core/src/db/migrations/mod.rs`; `crates/lazynote_core/src/repo/scoped_query_repo.rs`; `crates/lazynote_core/src/service/creation_service.rs`; `crates/lazynote_core/src/guard/mod.rs`; `crates/lazynote_ffi/src/api.rs`; `apps/lazynote_flutter/lib/core/workspace/workspace_tree_service.dart`; `apps/lazynote_flutter/lib/features/notes/note_explorer.dart` | `PR-0408` migration-first prerequisite is landed, `PR-0409` lands the scoped-query slice with an explicit compatibility bridge, `PR-0410` consumes that bridge for canonical creation/tree routing, `PR-0411` layers guarded FFI on top without reopening schema/service contracts, and `PR-0412` now lands the Flutter core slice plus a narrow smoke-driven Explorer operability bridge; broader `PR-0413` feature migration remains pending |
| `cutover-cleanup` | `PR-0411` and `PR-0413` | `partial` | `crates/lazynote_ffi/src/api.rs`; `docs/releases/v0.4/prs/PR-0411-guard-ffi.md`; `docs/releases/v0.4/prs/PR-0411A-ffi-structural-cleanup.md`; `docs/releases/v0.4/prs/PR-0413-flutter-features.md` | `PR-0411` lands the expand-stage bridge by converting the 15 legacy wrappers into thin guarded delegates; contract-stage removal stays owned by `PR-0413`, and structural cleanup stays owned by `PR-0411A` |
| `api-doc-ownership` | `PR-0411`, `PR-0413`, and governance audit | `partial` | `docs/api/ffi-contracts.md`; `docs/api/error-codes.md`; `docs/governance/API_COMPATIBILITY.md`; `docs/releases/v0.4/prs/PR-0411-guard-ffi.md` | `PR-0411` now documents the guarded caller contract, guarded export inventory, error-code additions, and expand-stage compatibility rules; later PRs still own consumer-migration and legacy-removal doc cleanup |
| `verification-gates` | `PR-0408` through `PR-0413` | `partial` | `crates/lazynote_core/tests/migration_0012_test.rs`; `crates/lazynote_core/tests/workspace_tree.rs`; `crates/lazynote_core/tests/db_migrations.rs`; `crates/lazynote_core/tests/scoped_query_repo.rs`; `crates/lazynote_core/tests/time_matrix.rs`; `crates/lazynote_core/tests/creation_service_routing.rs`; `crates/lazynote_core/tests/guard_test.rs`; `crates/lazynote_ffi/src/api.rs`; `docs/releases/v0.4/prs/PR-0411-guard-ffi.md`; `apps/lazynote_flutter/test/core/workspace/workspace_tree_service_test.dart`; `apps/lazynote_flutter/test/notes_controller_workspace_tree_guards_test.dart`; `apps/lazynote_flutter/test/explorer_context_actions_test.dart`; `apps/lazynote_flutter/test/note_explorer_tree_test.dart`; `apps/lazynote_flutter/test/workspace_integration_flow_test.dart`; `docs/releases/v0.4/prs/PR-0412-flutter-core.md` | `PR-0408` through `PR-0411` verification remains landed, and `PR-0412` now adds focused Flutter core workspace service coverage, replayed notes workspace guard regressions, focused Explorer concrete-root regressions, full `flutter analyze`, full `flutter test`, and repository architecture validation; broader `PR-0413` feature gates remain pending |
