# Docs Index

This is the canonical documentation entrypoint for LazyNote.

## Language Policy

- Canonical docs language is English.
- Translations are optional and may lag behind canonical docs.
- Internal links should always target canonical paths under `docs/`.

Translation header template (for future localized pages):

```md
> Translation of `docs/<canonical-path>.md` at commit `<source_commit_sha>`.
> This translation may lag behind the canonical English source.
```

## Start Here

- `README.md`
- `docs/governance/CONTRIBUTING.md`
- `docs/releases/v0.2/README.md`

## Architecture

- `docs/architecture/overview.md`
- `docs/architecture/data-model.md`
- `docs/architecture/note-schema.md`
- `docs/architecture/engineering-standards.md`

## API

- `docs/api/README.md`
- `docs/api/ffi-contracts.md`
- `docs/api/error-codes.md`

## Product and Planning

- `docs/product/vision.md`
- `docs/product/roadmap.md`
- `docs/product/milestones.md`
- `docs/releases/v0.2/README.md`

## Governance and Compliance

- `docs/governance/GOVERNANCE.md`
- `docs/governance/API_COMPATIBILITY.md`
- `docs/governance/plugin-capabilities.md`
- `docs/compliance/privacy.md`
- `docs/compliance/data-retention.md`

## Development

- `docs/development/windows-quickstart.md`
- `docs/development/windows.md`
- `docs/development/bug-archive.md`
